...
ifneq ($(OS), Windows_NT)
  ...
  # Make NVML support optional
  USE_NVML ?= 1
  ifeq ($(USE_NVML), 1)
    NVCC_LDFLAGS += -lnvidia-ml
    NVCC_FLAGS += -DUSE_NVML=1
  else
    NVCC_FLAGS += -DUSE_NVML=0
  endif
...
.PHONY: all train_gpt2 test_gpt2 train_gpt2cu test_gpt2cu train_gpt2fp32cu test_gpt2fp32cu profile_gpt2cu gpt2_inference
...
ifeq ($(NVCC),)
    ...
    TARGETS += train_gpt2cu test_gpt2cu train_gpt2fp32cu test_gpt2fp32cu gpt2_inference $(NVCC_CUDNN)
endif
...
gpt2_inference: gpt2_inference.cu $(NVCC_CUDNN)
	$(NVCC) $(NVCC_FLAGS) $(PFLAGS) $^ $(NVCC_LDFLAGS) $(NVCC_INCLUDES) $(NVCC_LDLIBS) $(CUDA_OUTPUT_FILE)
